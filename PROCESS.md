...
- **Dimensions first.** Measure the foreground element in `reference.png` (image viewer or browser devtools). Record:
...
- **Colors second.** Refine the `SAMPLE_POINTS` fractions in `iterate.js` so each sample lands on the element (not the backdrop), then run `node iterate.js sample`. This:
...
4. **Re-run `iterate.js`.**
5. **Regression guard (concrete mechanism):**
   - If the score **drops**, commit: `git add index.html && git commit -m "iter N: <score>% — <named target>"`.
   - If the score **goes up**, revert with `git checkout -- index.html`. Do not stack edits on a regression.
   - If the score barely moves (<0.5pp), do **not** commit. Rethink structurally (wrong shape, wrong gradient direction) — don't pile on.
6. **Review the trend** in `scores.log` after every run. The log is the authoritative progress record — if the trend flattens or reverses, stop and rethink.
...
### 8. Check in with user every 3 iterations
- After every 3 iterations of step 6, pause.
...
- Do not silently continue past 3 iterations without a check-in — diminishing returns and subjective "close enough" calls belong to the user, not me.
